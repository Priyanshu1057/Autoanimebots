...
class AniwaveScraper:
    ...
    def search_anime(self, query):
        ...
        try:
            ...
            if resp.status_code == 200:
                data = resp.json()
                # Aniwave returns {'status': 200, 'result': {'html': '...'}}
                html_content = ""
    ...
    def list_episodes(self, anime_id):
        ...
        """
        # If passed a slug like 'ore-dake-level-up-na-ken-76040', extract numeric ID or keep slug
        slug = str(anime_id).replace(self.BASE_URL, "").replace("/watch/", "").strip("/")
    ...
    def get_episode_servers(self, anime_id, ep_num):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                servers = []
                # Parse server blocks by type (sub, dub, kord)
    ...
    def get_home_sections(self):
        """
        Extract featured spotlight from the homepage.
        """
...
if __name__ == '__main__':
    ...
    print("Testing Aniwave search...")
    res = scraper.search_anime('solo leveling')
    print(f"Found {len(res)} results:")
    for item in res[:3]:
        print(f" - {item['title']} (ID: {item['id']}, URL: {item['url']})")

    if res:
        first = res[0]
        print(f"\nListing episodes for {first['title']} ({first['id']})...")
        eps = scraper.list_episodes(first['slug'])
        print(f"Found {len(eps)} episodes.")

        if eps:
            print(f"\nFetching servers for Episode {eps[0]['ep_number']}...")
            servers = scraper.get_episode_servers(first['slug'], eps[0]['ep_number'])
            print(f"Found servers: {[s['name'] + ' (' + s['type'] + ')' for s in servers]}")

            print(f"\nFetching source embed URL...")
            source = scraper.get_episode_sources(first['slug'], eps[0]['ep_number'])
            if source:
                print(f"Embed URL ({source['server']}): {source['embed_url']}")
